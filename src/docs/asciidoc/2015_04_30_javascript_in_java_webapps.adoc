...
* You need to copy the web artifaccts at build time to your deployment folder
...
** Zip together all needed files (e.g. all angularjs files). Then you can either copy and unzip the zip file during the build process into
the deployment folder.
** Java build (gradle/maven):
*** Or you could add the zip to your maven repository and reference it as normal dependency in the build process (TODO: show gradle example), TODO: webjar note.
** Javascript build (bower/npm/grunt/gulp):
*** The commercial version of Artifactory is able to serve Javascript artifacts: https://www.jfrog.com/confluence/display/RTF/Npm+Repositories.
*** You can setup a https://www.npmjs.com/package/private-bower[private bower repository]
or a https://www.google.ch/webhp?&ie=UTF-8#q=private+npm+repository[private npm repository] or both.
