...
When I tell developers about opendolphin.js very often they ask me if it is available via bower.
As a Java developer I also work with Javascript frameworks like AngularJS and other javascript libraries and frameworks.
I am quite familiar with the tools in the Java area but only a mere overview what exists in the Javascript world.

I tried out the build tools grunt and gulp (corresponding to maven and gradle),
the dependency management tools npm and bower (maven dependency management built into maven and gradle).
In addition I played around with minification and sourcemaps.

To be able to answer the bower question in a profound way I decided to have a closer look at these tools.
...
* Execute `node node_modules/gulp/bin/gulp.js`

Now there is a new folder `runtime` (which you should add to .gitignore) containing `jquery/2.1.3/jquery.min.js`
