...
==== Start simple (from a Java developer point of view)
...
Note that this is exactly the same behavior as many Java developers did before maven repositories did exist.
It works but is not a very good idea. Why?
Even with a checked in jar file it is not clear if it really is exactly the same as the one found on maven central.
...
When you updating the library it also makes sense to use a version number in the foldername you checked in the library.
...
*Without access to the internet* you need to provide a place from where you can serve these artifacts. For this example
...
Since this URL is not what you would like the browser to use your build script should include a copy step which would copy
...
* Without internet acdess a private bower repository needs to be setup. You could argue that you could simply download the artifacts,
...
with many dependencies with transitive dependencies this is becoming very hard to handle.
